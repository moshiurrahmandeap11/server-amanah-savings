    ...
    const [
      ...
      depositsCollection
        .aggregate([
          ...
          { $group: { _id: null, total: { $sum: "$depositAmount" } } }, // Fixed: use depositAmount
        ])
        .toArray()
        .then((r) => r[0]?.total || 0),
      withdrawalsCollection
        .aggregate([
          ...
          { $group: { _id: null, total: { $sum: "$withdrawalAmount" } } }, // Fixed: use withdrawalAmount
        ])
        .toArray()
        .then((r) => r[0]?.total || 0),
      ...
      data: {
        ...
        recent: {
          ...
          deposits: recentDeposits.map((d) => ({
            id: d._id,
            userId: d.userId,
            amount: d.depositAmount, // Fixed: use depositAmount
          ...
          withdrawals: recentWithdrawals.map((w) => ({
            id: w._id,
            userId: w.userId,
            amount: w.withdrawalAmount, // Fixed: use withdrawalAmount
...
export const getReportsData = async (req, res) => {
  try {
    ...
    const topSavers = await depositsCollection
      ...
      .toArray();
// In admin.controller.js, replace the kpis object with an array:
return res.status(200).json({
  success: true,
  data: {
    kpis: [
      {
        ...
        value: `৳${((totalDepositsAmount - totalWithdrawalsAmount) || 0).toLocaleString()}`,
        valueEn: `৳${((totalDepositsAmount - totalWithdrawalsAmount) || 0).toLocaleString()}`,
        icon: "💰",
        bg: "bg-primary/10",
        color: "primary",
        change: "+12%",
      ...
      },
      {
        ...
        valueEn: newUsers.toLocaleString(),
        icon: "👥",
        bg: "bg-accent/10",
        color: "accent",
        change: "+8%",
      ...
      },
      {
        ...
        valueEn: activeUsers.toLocaleString(),
        icon: "📱",
        bg: "bg-success/10",
        color: "success",
        change: "+5%",
      ...
      },
      {
        ...
        valueEn: activeGoals.toLocaleString(),
        icon: "🎯",
        bg: "bg-warning/10",
        color: "warning",
        change: "+15%",
...
export const getAnalyticsData = async (req, res) => {
  try {
    ...
    const deviceBreakdown = deviceStats.map((d, idx) => ({
      name: d._id || "Unknown",
      count: d.count,
    ...
    const divisionStats = await usersCollection
      .aggregate([
           { $match: { division: { $exists: true, $nin: [null, "", undefined] } } },
    ...
    const divisionBreakdown = divisionStats.map((d, idx) => ({
      name: d._id,
      count: d.count,
      percentage: Math.round((d.count / totalDivisions) * 100),
      color: getDivisionColor(idx),
    }));
    
    // Session Stats (for stats cards)
    const totalUsers = await usersCollection.countDocuments({ role: "user" });
    const activeToday = await loginHistoryCollection.countDocuments({
      loginTime: { $gte: today },
      success: true,
    });
    
    // Today's deposits - FIXED: Use depositAmount instead of amount
    const todayDeposits = await depositsCollection.aggregate([
      { $match: { createdAt: { $gte: today }, status: "approved" } },
      { $group: { _id: null, total: { $sum: "$depositAmount" } } },
    ]).toArray();
    const todayDepositAmount = todayDeposits[0]?.total || 0;
    ...
    const sessionStats = [
      ...
      },
      {
        ...
        label: "Active Today",
        trend: activeToday > 0 ? "+5%" : "0%",
        trendUp: true,
        bg: "bg-blue-500/10",
      },
      {
        icon: "💰",
        value: todayDepositAmount >= 100000 
          ? `৳${(todayDepositAmount / 100000).toFixed(1)}L` 
          : `৳${todayDepositAmount.toLocaleString()}`,
        label: "Today's Deposit",
        trend: todayDepositAmount > 0 ? "+18%" : "0%",
        trendUp: true,
        bg: "bg-green-500/10",
      },
      {
    ...
    const avgSessions = await loginHistoryCollection
      .aggregate([
        { $match: { loginTime: { $gte: last14Days }, success: true } },
        {
          $group: {
    ...
    const activeLast7Days = await loginHistoryCollection
      .aggregate([
    ...
    const totalActiveUsers = await usersCollection.countDocuments({ accountActive: true });
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        sessionStats,
        additionalStats: {
          avgSessionsPerUser: Math.round(avgSessions[0]?.avgSessions || 0),
          activeUsersLast7Days: activeLast7Days[0]?.count || 0,
          retentionRate: totalActiveUsers > 0
            ? Math.round(((activeLast7Days[0]?.count || 0) / totalActiveUsers) * 100)
            : 0,
        },
      },
    });
...
export const getSavingsData = async (req, res) => {
  try {
    ...
    const formattedGoalsByType = goalsByType.map((g) => ({
      ...
      avgMonthly:
        g.totalCurrent > 0 && g.count > 0
          ? Math.round(g.totalCurrent / g.count)
          : 0,
    }));
    return res.status(200).json({
      success: true,
  ...
  try {
    ...
    const alerts = [
      ...
      ...unusualDeposits.map((u) => ({
        ...
        description: `${u.user?.fullName || "Unknown"} made ${u.count} deposits (৳${u.totalAmount}) in 24h`,
    ...
    if (type === "all" || type === "deposit") {
      ...
      for (const d of deposits) {
        ...
        transactions.push({
          id: d._id,
          type: "deposit",
          transactionId: d.transactionId || d._id.toString(),
          amount: d.amount,
          status: d.status,
          method: d.method,
          userId: d.userId,
